...
const OilSaleModal = ({ onClose, onSave, editingSale }) => {
  ...
  useEffect(() => { 
    if (editingSale) { 
      ...
      setFormData({ ...editingSale, date: !isNaN(d.getTime()) ? d : new Date() }); 
    } 
  }, [editingSale]);
  ...
  const handleSubmit = (e) => { 
    e.preventDefault(); 
    onSave({ ...formData, totalRevenue, id: editingSale?.id }); 
    onClose(); 
  };
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-xl p-6">
        <h2 className="text-2xl font-bold mb-6 text-emerald-800">
          {editingSale ? 'Zeytinyağı Satışını Düzenle' : 'Yeni Zeytinyağı Satışı Ekle'}
        </h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <FormField label="Müşteri Adı" id="customerName" type="text" name="customerName" value={formData.customerName} onChange={handleChange} required className="block w-full border border-gray-300 rounded-md shadow-sm py-3 px-3 min-h-[48px]" />
          ...
          <div className="bg-gray-50 p-3 rounded-md border text-center">
            <span className="block text-xs font-semibold text-gray-500 mb-1">Hesaplanan Satış Geliri</span>
