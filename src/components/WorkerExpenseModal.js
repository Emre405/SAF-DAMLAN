    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-xl p-6">
        <h2 className="text-2xl font-bold mb-6 text-emerald-800">
          {editingExpense ? 'İşçi Harcamasını Düzenle' : 'Yeni İşçi Harcaması Ekle'}
        </h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <FormField label="İşçi Adı" id="workerName" type="text" name="workerName" value={formData.workerName} onChange={handleChange} required className="block w-full border border-gray-300 rounded-md shadow-sm py-3 px-3 min-h-[48px]" />
          <FormField label="Çalıştığı Gün Sayısı" id="daysWorked" type="number" inputMode="numeric" name="daysWorked" value={formData.daysWorked} onChange={handleChange} className="block w-full border border-gray-300 rounded-md shadow-sm py-3 px-3 min-h-[48px]" />
          <FormField label="Verilen Ücret (₺)" id="amount" type="number" inputMode="decimal" name="amount" value={formData.amount} onChange={handleChange} required className="block w-full border border-gray-300 rounded-md shadow-sm py-3 px-3 min-h-[48px]" />
          <TextAreaField label="Açıklama" id="description" name="description" value={formData.description} onChange={handleChange} className="block w-full border border-gray-300 rounded-md shadow-sm py-3 px-3" rows="3" /> 
...
export default WorkerExpenseModal;
